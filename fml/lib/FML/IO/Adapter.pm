...
package FML::IO::Adapter;
use vars qw(@ISA @ORIG_ISA $FirstTime $AUTOLOAD);
use strict;
use Carp;
use FML::IO::Adapter::ErrorStatus qw(error_set error error_clear);

my $debug = 0;

# prepare error_set() is called
@ISA = qw(FML::IO::Adapter::ErrorStatus);
...
FML::IO::Adapter - adapter for several IO interfaces.

=head1 SYNOPSIS

    use FML::IO::Adapter;
    $obj = new FML::IO::Adapter ($map, $map_params);
...
FML::IO::Adapter converts data in arbitrary map e.g. file, /etc/group,
RDBMS into this structure described above.
Also,
FML::IO::Adapter provides unified access methods to this structure.


Instead of unification, FML::IO::Adapter may provide amibugous IO.
...
sub new
{
	...
	if ($map =~ /file:(\S+)/ || $map =~ m@^(/\S+)@) {
	    $me->{_file} = $1;
	    $me->{_type} = 'file';
	    $pkg         = 'FML::IO::Adapter::File';
	}
	elsif ($map =~ /pcre:(\S+)/) {
	    $me->{_file} = $1;
	    $me->{_type} = 'file';
	    $pkg         = 'FML::IO::Adapter::PCRE';
	}
	elsif ($map =~ /unix\.group:(\S+)/) {
	    $me->{_name} = $1;
	    $me->{_type} = 'unix.group';
	    $pkg         = 'FML::IO::Adapter::UnixGroup';
	}
	elsif ($map =~ /nis\.group:(\S+)/) {
	    $me->{_name} = $1;
	    $me->{_type} = 'nis.group';
	    $pkg         = 'FML::IO::Adapter::NIS';
	}
	    ...
	    if ($map =~ /mysql:/i) {
		$pkg = 'FML::IO::Adapter::MySQL';
	    }
	    elsif ($map =~ /postgresql:/i) {
		$pkg = 'FML::IO::Adapter::PostgreSQL';
	    }
	    @pkg           = qw(FML::IO::Adapter::DBI);
	}
	elsif ($map =~ /(ldap):(\S+)/i) {
	    $me->{_type}   = $1;
	    $me->{_schema} = $2;
	    $me->{_params} = $args;
	    $me->{_type}   =~ tr/A-Z/a-z/; # lowercase the '_type' syntax
	    $pkg           = 'FML::IO::Adapter::LDAP';
	}
	else {
	    my $s = "FML::IO::Adapter::new: map='$map' is unknown.";
	    error_set($me, $s);
	}
    }
...
sub find
{
    ...
    # XXX PLEASE CARE REGEXP BEFORE CALL FML::IO::Adapter.
...
FML::IO::Adapter first appeared in fml8 mailing list driver package.
