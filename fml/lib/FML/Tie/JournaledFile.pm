...
FML::Tie::JournaledFile - hash operation for a jurnaled style log file.

=head1 SYNOPSIS

   use FML::Tie::JournaledFile;
   $db = new FML::Tie::JournaledFile { file => 'cache.txt' };
...
or access in hash style
   ...
   use FML::Tie::JournaledFile;
   tie %db, 'FML::Tie::JournaledFile', { file => 'cache.txt' };
   print $db{ rudo }, "\n";
...
sub new
{
	...
	if ($condition eq 'first' || $condition eq 'last') {
	    ...
	    croak("FML::Tie::JournaledFile: invalid condition");
	}
    }
...
sub _fetch
{
  ...
  LINE:
	...
	croak("FML::Tie::JournaledFile: invalid mode");
    }
}
...
FML::Tie::JournaledFile first appeared in fml8 mailing list driver package.
