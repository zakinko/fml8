...
FML::Tie::JournaledDir - tie hash to journaled style files in a directory.

=head1 SYNOPSIS

    use FML::Tie::JournaledDir;
    tie %db, 'FML::Tie::JournaledDir', { dir => '/some/where' };
    $db{ 'key' } = 'value';
    untie %db

=head1 DESCRIPTIONS

tie hash by C<FML::Tie::JournaledDir> acceses some directory holding a lot
...
C<FML::Tie::JournaledFile> called from C<FML::Tie::JournaledDir> manipulates
each file.

C<FML::Tie::JournaledDir> has cache files in a directory.
C<FML::Tie::JournaledDir> wraps C<FML::Tie::JournaledFile> over several files.
...
It uses C<FML::Tie::JournaledFile> in background.
...
sub new
{
    ...
    # sanity.
    unless ($dir) { croak("FML::Tie::JournaledDir: dir unspecified");}
...
#               by FML::Tie::JournaledFile sequentially.
...
sub FETCH
{
  ...
  FILE:
	...
	if (-f $f) {
	    ...
	    my $obj = new FML::Tie::JournaledFile {
...
#               by FML::Tie::JournaledFile.
#    Arguments: OBJ($self) STR($key) STR($value)
# Side Effects: none
# Return Value: STR(FML::Tie::JournaledFile->STORE() operation return value)
sub STORE
{
    my ($self, $key, $value) = @_;
...
if ($0 eq __FILE__) {
    ...
    my $tie = new FML::Tie::JournaledDir {
	dir => $dir,
    };
    $tie->expire();
}
...
FML::Tie::JournaledDir first appeared in fml8 mailing list driver package.
