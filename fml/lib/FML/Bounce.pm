...
FML::Bounce - analyze error message.
...
For non DSN pattern,
try to analyze it by using modules in C<FML::Bounce::>
...
for C<FML::Bounce::> modules, for example, C<FML::Bounce::DSN>.

C<FML::Bounce::$model::analyze( \$msg, \$result )>
...
sub address_cleanup
{
    ...
    # FML::Bounce::FixBrokenAddress class provides irrgular
    # address handlings, so handles domain/MTA specific addresses.
    # For example, nifty.ne.jp, webtv.ne.jp, ...
    # XXX-TODO: FML::Bounce::FixBrokenAddress::FixIt is ugly.
    use FML::Bounce::FixBrokenAddress;
    return FML::Bounce::FixBrokenAddress::FixIt($hint, $addr);
}


=head1 SECURITY CONSIDERATION

The address returned by FML::Bounce class may be unsafe.  Please
...
FML::Bounce first appeared in fml8 mailing list driver package.
