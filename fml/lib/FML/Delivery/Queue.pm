...
package FML::Delivery::Queue;
use strict;
use Carp;
use vars qw(@ISA $Counter $RandCounter @class_list @local_class_list);
use File::Spec;

# FML::Delivery::Queue IS-A FML::Delivery::Base.
...
FML::Delivery::Queue - handle mail queue system.
...
=head1 SYNOPSIS
    ...
    my $queue = new FML::Delivery::Queue { directory => "/some/where" };
...
C<FML::Delivery::Queue> provides basic manipulation of mail queue.
...
C<FML::Delivery>.
C<FML::Delivery::Queue> manipulates only queue around things.
...
sub lock
{
    ...
    use FML::IO::Adapter;
    my $io = new FML::IO::Adapter $lockfile;
    if (defined $io) {
...
sub set_sleep_count
{
    ...
    use FML::IO::Adapter;
    my $io = new FML::IO::Adapter $strategy_file;
    $io->touch();
    $io->open();
    $io->delete("SLEEP");
...
FML::Delivery::Queue first appeared in fml8 mailing list driver package.
