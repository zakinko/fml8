...
package FML::Delivery::SMTP;
use strict;
use vars qw(@ISA @EXPORT @EXPORT_OK);
use Carp;
use IO::Socket;

# FML::Delivery::SMTP IS-A FML::Delivery::Protocol.
use FML::Delivery::Protocol;
@ISA = qw(FML::Delivery::Protocol);
...
FML::Delivery::SMTP - interface for SMTP service
...
To initialize,
    ...
    my $service = new FML::Delivery::SMTP {
        log_function       => $fp,
...
sub-classes,
C<FML::Delivery::Net::INET4> and
C<FML::Delivery::Net::INET6>.

It sends a list of all recipients indicated by $recipient_maps.
C<FML::IO::Adapter> resolves $recipient_maps and provides the abstract
IO layer. It provides the usual file IO methods for each C<map>.
See L<FML::IO::Adapter> for more details.
...
# Descriptions: FML::Delivery::SMTP constructor.
...
#
#    Arguments: OBJ($self) HASH_REF($args)
# Side Effects: See FML::Delivery::Utils for recipient_map utilities
...
sub deliver
{
  ...
  MAP:
	...
	eval q{
	    use FML::IO::Adapter;
	    my $obj = new FML::IO::Adapter $map, $args->{ map_params };
	    if (defined $obj) {
...
#                                         unix.group:admin
#                                         mysql:toymodel
#               FML::IO::Adapter class is essential to handle abstract
#               $recipient_map.
#    Arguments: OBJ($self) HASH_REF($args)
...
L<IO::Socket>,
L<FML::Delivery::Utils>,
L<FML::Delivery::INET4>,
L<FML::Delivery::INET6>,
L<FML::IO::Adapter>
...
FML::Delivery::SMTP first appeared in fml8 mailing list driver package.
